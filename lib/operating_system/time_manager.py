...
class TimeManager:
    ...
    def move_time(self, days: int = 0, hours: int = 0, minutes: int = 5, seconds: int = 0) -> None:
        ...
        delta = (start_point + timedelta(days=days, hours=hours, minutes=minutes, seconds=seconds + 1)) \
    ...
    def is_network_time_used(self) -> bool:
        result = self.executor.sudo_get_output('systemsetup -getusingnetworktime').split(':')[-1].strip().lower()
