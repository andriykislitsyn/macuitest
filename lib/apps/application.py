...
from lib.operating_system.plist_helper import PlistHelper
...
class Application:
    ...
    def activate(self):
        ...
        assert self.frontmost is True

    def set_window_position(self, position: str = '{1210, 670}'):
        if self.frontmost:
            try:
                return as_wrapper.tell_app_process(f'set position of the front window to {position}', self.name)
            except AppleScriptError:
                pass
    ...
    def set_window_size(self, size: str = '{700, 400}'):
        if self.frontmost:
            try:
                return as_wrapper.tell_app_process(f'set size of the front window to {size}', self.name)
            except AppleScriptError:
                pass
