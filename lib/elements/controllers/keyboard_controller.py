...
from lib.elements.controllers.keyboard_mappings import KEYBOARD_KEYS, SPECIAL_KEYS
...
class KeyBoardController:
    ...
    def write(self, message: str, pause: float = .001):
    ...
    def send_regular_key_event(self, key: str, event_type):
        ...
        if self.is_shift_char(key):
            ...
            event = Quartz.CGEventCreateKeyboardEvent(None, KEYBOARD_KEYS['shift'], event_type == 'down')
            Quartz.CGEventPost(Quartz.kCGHIDEventTap, event)
            time.sleep(.03)  # Tiny sleep to let OS X catch up on us pressing shift
        else:
