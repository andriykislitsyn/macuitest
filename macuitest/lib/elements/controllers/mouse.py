...
class Mouse:
    ...
    def paste(self, x: int, y: int, phrase: str = '') -> None:
        ...
        self.click(x, y)
        keyboard.write(phrase, pause=.02)
    ...
    def click(self, x: int, y: int, hold: float = MouseConfig.hold, duration: float = MouseConfig.move,
              pause: float = MouseConfig.pause) -> None:
        """Hover over position and left-click once."""
        time.sleep(pause)
        self.hover(x, y, duration)
        self._press_mouse_button(x, y, mouse_button='left', hold=hold, pause=.125)

    def right_click(self, x: int, y: int, hold: float = MouseConfig.hold, duration: float = MouseConfig.move,
                    pause: float = MouseConfig.pause) -> None:
        """Hover over the position and control-click once."""
        time.sleep(pause)
        self.hover(x, y, duration)
        self._press_mouse_button(x, y, mouse_button='right', hold=hold, pause=.125)
